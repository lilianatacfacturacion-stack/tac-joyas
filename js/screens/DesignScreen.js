...
export class DesignScreen {
  ...
  render() {
    ...
    // Piezas incluidas para poder eliminar zona
    const keptParts = (project.selectedParts || project.detectedParts || []).filter(p => p.kept);
    ...
    el.innerHTML = `
      ...
      <div class="screen-body" style="padding:8px 12px;">
        ...
        <div style="font-size:0.75rem; color:var(--text-muted); margin-bottom:8px;">
          Borra del grabado la zona correspondiente a una pieza detectada (ej: enganche, barra).
        </div>
  ...
  _setupEvents(el, project) {
    ...
    // Borrar zona de pieza detectada completa
    el.querySelectorAll('[data-erase-part]').forEach(btn => {
      btn.onclick = () => {
        ...
        this._saveHistory();
        this._drawWorkCanvas(el);
        this.app.showToast(`Zona "${part.label}" borrada`);
      };
    });
